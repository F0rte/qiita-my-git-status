...
const getGitStatus = (): string => {
  ...
  for (const file of allFiles) {
    ...
    if (!indexHash && workHash) {
      ...
      if (indexHash !== workHash) {
        if (!workHash) {
          notStaged.set(file, "deleted");
        } else if (!headHash) {
          notStaged.set(file, "new file");
        } else {
          notStaged.set(file, "modified");
        }
      }
      if (indexHash !== headHash) {
        if (!indexHash) {
          toBeCommited.set(file, "deleted");
        } else if (!headHash) {
          toBeCommited.set(file, "new file");
        } else {
          toBeCommited.set(file, "modified");
        }
      }
    }
  }
